<!-- <div align="center">
  <img src="/微信图片_20250820103901_6082_53.jpg" height="300"/>
</div> -->

<h1 align="center">👋 你好，我是赵岩</h1>

<p align="center">
  <em>Android 开发工程师 | 移动端技术爱好者 | 追求极致用户体验</em>
</p>

<p align="center">
  🎤  🕺  🎶  🏀  🐔  👏
</p>

## 🚀 关于我

- 📱 专注于 Android 原生开发，热爱构建流畅优雅的移动应用
- 🎨 熟练使用 Jetpack Compose 构建现代化声明式 UI
- 🦋 精通 Flutter 跨平台开发，一套代码多端运行
- 💡 追求代码质量，注重性能优化和用户体验
- 🌱 持续学习新技术，紧跟移动开发前沿趋势
- 🤝 乐于分享技术经验，开放合作交流
- ⚡ 热爱音乐、篮球和各种有趣的事物

## 🛠️ 技术栈

**移动开发:**
- Android (Kotlin/Java)
- Jetpack Compose - 声明式 UI 开发
- Flutter & Dart - 跨平台解决方案
- Android Architecture Components (MVVM、MVI)

**核心技能:**
- Jetpack 组件库 (ViewModel, LiveData, Room, Navigation, Hilt)
- Material Design 3 设计规范
- 协程 & Flow 响应式编程
- Retrofit、OkHttp 网络请求
- Glide/Coil 图片加载

## 📊 GitHub 统计

|   <img src="/微信图片_20250820103901_6082_53.jpg" height="200"/> | <a href="https://github.com/anuraghazra/github-readme-stats"><img align="center" src="https://github-readme-stats.vercel.app/api?username=Zhao-Yan-Yan&show_icons=true&include_all_commits=true&theme=buefy&hide_border=true" alt="Anurag's github stats" /></a> | <a href="https://github.com/anuraghazra/github-readme-stats"><img align="center" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Zhao-Yan-Yan&layout=compact&theme=buefy&hide_border=true" /></a> |
| ------------- | ------------- | ------------- |

<!-- <div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Zhao-Yan-Yan&show_icons=true&theme=radical&hide_border=true&locale=cn" alt="GitHub Stats" />
</div>

<div align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=Zhao-Yan-Yan&theme=radical&hide_border=true&locale=zh_Hans" alt="GitHub Streak" />
</div>

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Zhao-Yan-Yan&layout=compact&theme=radical&hide_border=true&locale=cn" alt="Most Used Languages" />
</div> -->

## 📫 联系我

<p align="center">
  欢迎与我交流移动开发技术，一起进步！
</p>
<!-- <p align="center">
  <img src="https://komarev.com/ghpvc/?username=Zhao-Yan-Yan&color=blueviolet&style=flat-square&label=访问量" alt="Profile Views" />
</p> -->
